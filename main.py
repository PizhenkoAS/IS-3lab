...
if args.generation is not None:
    ...
    # генерация ключа симметричного алгоритма шифрования SEED - ключ 128 битов
    ...
    print(colored('1.3. Сериализовать ассиметричные ключи.', 'green'))
    ...
    print(colored("1.4. Зашифровать ключ симметричного шифрования открытым ключом и сохранить по указанному пути.",
                  'green'))
    ...
    print('Сохранил шифрование симметричного ключа в файл ' + json_data['symmetric_key'])
    ...
    # шифрование текста симметричным алгоритмом
    iv = os.urandom(16)  # случайное значение для инициализации блочного режима, должно быть размером с блок и
    # каждый раз новым
    # cохранить случайное значение в файл
    with open(json_data['random_value'], 'wb') as file_value:
        file_value.write(iv)
    cipher = Cipher(algorithms.SEED(dc_symmetric_key), modes.CBC(iv))
